...
tooltipAndText = () => ({
    callbacks: {
        label: (context) => {
            ...
            return [label, text]; // returns an array of strings
        }
    }
});
...
const teamLines = (data,thisGame,id) => { 
    ...
    let sr = team.map(({play_num, team_sr, text}) => ({ x: play_num, y: team_sr , text}));
    let xr = team.map(({play_num, team_xr, text}) => ({ x: play_num, y: team_xr , text}));
...
const typeLines = (data,thisGame,id,teamNum) => {
        
    let game = data.filter(({game}) => game == thisGame);
    let chart = game.filter(({chart}) => chart == 'plays');
    let team = chart.filter(({team_num}) => team_num == teamNum);
    let labels = team.map(a => a.play_num);
    let rushes = team.filter(({play_type}) => play_type == "Rush");
    let passes = team.filter(({play_type}) => play_type == "Pass");
    
    let playsMax = Math.max.apply(Math, team.map(({play_num}) => play_num));
    let newQuarters = [...new Set(team.map(a => a.quarter))];
    quarterMarker(newQuarters,team,playsMax,"play_num");

    let rushSr = rushes.map(({play_num, sr, text}) => ({ x: play_num, y: sr, text }));
    let passSr = passes.map(({play_num, sr, text}) => ({ x: play_num, y: sr, text }));
...
// RUSH RATE
const rushRate = (data,thisGame,id,teamNum) => {
        
    let game = data.filter(({game}) => game == thisGame);
    let chart = game.filter(({chart}) => chart == 'plays');
    let team = chart.filter(({team_num}) => team_num == teamNum);
    let labels = team.map(a => a.play_num);
    let rushRate = team.map(({play_num, rr, text}) => ({ x: play_num, y: rr, text }));
...
const srXrBars = (data,thisGame,id,teamNum,column) => {
    ...
    let sr = team.map(a => ({value: a.sr, count: a.count}));
    let xr = team.map(a => ({value: a.xr, count: a.count}));
    let srOpp = opponent.map(a => ({value: a.sr, count: a.count}));
    let xrOpp = opponent.map(a => ({value: a.xr, count: a.count}));

    
    let dataset = (d,s,l,color) => ({
        data: d.map(a => a.value),
        stack: s,
        label: l,
        backgroundColor: color,
        datalabels: {
            formatter: (value, context) => {
                let count = d[context.dataIndex].count;
                if (context.dataset.label.includes('SR')) {
                    return count;
                } else {
                    return null;
                }
