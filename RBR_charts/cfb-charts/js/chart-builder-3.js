...
const typeLines = (data,thisGame,id,teamNum,column) => {
    ...
    let rushSr = rushes.map(({team_play, sr, text}) => ({ x: team_play, y: sr, text }));
    let passSr = passes.map(({team_play, sr, text}) => ({ x: team_play, y: sr, text }));
...
const rushRate = (data,thisGame,id,teamNum,column) => {
    ...
    let team = chart.filter(({team_num}) => team_num == teamNum);
    let labels = team.map(a => a.team_play);
    let rushRate = team.map(({team_play, rr, text}) => ({ x: team_play, y: rr, text }));
